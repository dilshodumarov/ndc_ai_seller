...
# AI Seller API - Authentication System

This repository contains a comprehensive authentication system for the AI Seller API. The system includes user registration, login, password management, and JWT-based authorization.
...
3. System sends a verification code to user's email
4. User submits the verification code
5. System creates the account and returns JWT tokens

### Login

1. User submits email and password
2. System validates credentials
3. System returns JWT tokens (access and refresh)

### Password Reset

1. User requests password reset with email
2. System sends a verification code to user's email
3. User submits verification code and new password
...
- `POST /api/v1/auth/refresh-token/:token` - Refresh access token
- `POST /api/v1/auth/forgot-password` - Request password reset
- `POST /api/v1/auth/verify-forgot-password` - Verify reset code and update password
- `POST /api/v1/auth/update-password` - Update password (authenticated)
- `DELETE /api/v1/auth/delete/:id` - Delete user account (admin only)
...
curl -X POST http://localhost:8080/api/v1/auth/register \
  ...
  -d '{"name":"John","surname":"Doe","email":"john@example.com","phone_number":"+998123456789","password":"Password123"}'
...
- Refresh tokens have a longer expiration time but are single-use
